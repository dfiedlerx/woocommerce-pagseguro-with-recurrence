...
class WC_PagSeguro_API {
	...
	protected function get_payment_xml( $order, $posted ) {
		...
		if ($posted['pagseguro_payment_method'] == 'recorrence') {

		    $method = $posted['pagseguro_payment_method'];
		...
		if ( 'creditCard' == $method || 'recorrence' == $method ) {
			...
			if ($method != 'recorrence') {

			    $installment['quantity'] =
                    isset( $posted['pagseguro_card_installments'] )
                        ? absint( $posted['pagseguro_card_installments'] )
                        : '';
                ...
                $holder_data       = array(
                    'name'       => isset( $posted['pagseguro_card_holder_name_recorrence'] ) ? sanitize_text_field( $posted['pagseguro_card_holder_name'] ) : '',
                    'cpf'        => isset( $posted['pagseguro_card_holder_cpf_recorrence'] ) ? sanitize_text_field( $posted['pagseguro_card_holder_cpf'] ) : '',
                    'birth_date' => isset( $posted['pagseguro_card_holder_birth_date_recorrence'] ) ? sanitize_text_field( $posted['pagseguro_card_holder_birth_date'] ) : '',
                    'phone'      => isset( $posted['pagseguro_card_holder_phone_recorrence'] ) ? sanitize_text_field( $posted['pagseguro_card_holder_phone'] ) : '',
                );
	...
	public function suspendeAtivaAssinatura ($idAssinatura) {
        ...
        try {
            ...
            if (get_post_meta($idAssinatura, 'active', true) == '1') {

                $enable = false;

            }

            $pagseguro->setHabilitarAssinatura($assinaturas->post_title, $enable);
    ...
    public function cancelaAssinatura ($idAssinatura) {
        ...
        try {
            ...
            $pagseguro->cancelarAssinatura($assinaturas->post_title);

            update_post_meta($idAssinatura, 'cancelled', 1);
            update_post_meta($idAssinatura, 'active',  0);
	...
	public function getTotalAmount ($items, $shippingPrice) {

	    $total = $shippingPrice;

	    if (isset($items['amount']) && isset($items['quantity'])) {
            ...
            foreach ($items as $currentItem) {
                ...
                $total +=  $currentItem['amount'] * $currentItem['quantity'];
	...
	public function recorrencePlan ($posted, $xml, $order_id) {
	    ...
	    $totalAmount = $this->getTotalAmount($xml['items']['item'], !empty($xml['shipping']['cost']) ? $xml['shipping']['cost'] : 0);

	    // PRIMEIRO SERÁ CRIADO O PLANO INDIVIDUAL PARA O CLIENTE //
        /*
         *  A taxa de adesão foi adicionada como um pagamento para a primeira entrega, enquanto que as parcelas
         * do plano serão referentes as entregas seguintes. Por causa disso a taxa de adesão sempre será igual a taxa
         * de manutenção mensal pois refere-se ao preço do produto.
         *
         * Outro ponto é em relação à duração do plano, como a taxa mensal é referente ao preço do produto, e este pode
         * variar, foi-se pensado uma duração máxima de 6 meses requerendo ao final que o cliente refaça o pagamento
         * recorrente pelo site.
         *
         * O prazo de pagamento e re-cobrança geralmente é fixo em outros modelos de negócio, mas nesse caso, optamos
         * por colocar um prazo personalizavel para melhor atender as necessidades do cliente e de seu pet.
         *
         */

        $pagseguro = new PagSeguroAssinaturas($accessInfo['email'], $accessInfo['token'], $sandBox);

        //O nome do plano irá seguir com o id gerado para o pedido
        $pagseguro->setReferencia('Plano_' . $order_id);

        //Cria uma descrição para o plano
        $pagseguro->setDescricao
        (
            'Pedido: ' . $order_id . ' / ' .
            'Cliente: ' . $posted['pagseguro_card_holder_name_recorrence'] . ' / ' .
            'Pet: ' . $posted['nome_do_seu_peludinho'] . ' / ' .
            'CPF: ' . $posted['billing_cpf']
        );

        //Valor a ser cobrado a cada renovação
        $pagseguro->setValor($totalAmount);

        //De quanto em quanto tempo será realizado uma nova cobrança (MENSAL, BIMESTRAL, TRIMESTRAL, SEMESTRAL, ANUAL)

        if ($posted['pagseguro_card_recorrence-time'] == 'bimestral') {

            $pagseguro->setPeriodicidade(PagSeguroAssinaturas::BIMESTRAL);

        } else if ($posted['pagseguro_card_recorrence-time'] == 'trimestral') {
         ...
         * Definido em 1 pois o plano é individual do cliente baseado em suas compras
        */
        $pagseguro->setMaximoUsuariosNoPlano(1);

       	$pagseguro->setNotificationURL(WC()->api_request_url( 'WC_PagSeguro_Gateway' ));
        $pagseguro->setURLCancelamento(WC()->api_request_url( 'WC_PagSeguro_Gateway' ));
        $pagseguro->setRedirectURL(WC()->api_request_url( 'WC_PagSeguro_Gateway' ));


        //=== Cria o plano ===//
        $recorrencePlanCode = $pagseguro->criarPlano();

        //Agora com o plano criado, iremos validar os dados do cartão do usuário e finalizar o pagamento de recorrência
        //Nome do comprador igual a como esta no CARTÂO
        $pagseguro->setNomeCliente($posted['pagseguro_card_holder_name_recorrence']);
        //Email do comprovador
        $pagseguro->setEmailCliente($posted['billing_email']);
        //Informa o telefone DD e número
        $telefone = str_replace('-', '', filter_var($posted['pagseguro_card_holder_phone_recorrence'], FILTER_SANITIZE_NUMBER_INT));
        ...
        $pagseguro->setEnderecoCliente
        (
            $posted['billing_address_1'],
            $posted['billing_number'],
            $posted['billing_address_2'],
            $posted['billing_neighborhood'],
            $posted['billing_city'],
            $posted['billing_state'],
            $posted['billing_postcode']
        );
        //Informa o ano de nascimento
        $pagseguro->setNascimentoCliente($posted['pagseguro_card_holder_birth_date_recorrence']);
        ...
        $pagseguro->setPlanoCode($recorrencePlanCode);
	...
	public function do_payment_request( $order, $posted, $order_id ) {
		 ...
		 */
		if ( ! in_array( $payment_method, $this->get_available_payment_methods() )  && $payment_method != 'recorrence') {
			return array(
				'url'   => '',
				'data'  => '',
        ...
        //Personalização referente ao modo de pagamento por recorrência
		if ($payment_method == 'recorrence') {

            $xmlArray = simplexml_load_string($xml, "SimpleXMLElement", LIBXML_NOCDATA);
            $xmlArray = json_encode($xmlArray);
            $xmlArray = json_decode($xmlArray,TRUE);

		    $response = $this->recorrencePlan($posted, $xmlArray, $order_id);
