...
class WC_PagSeguro_Gateway extends WC_Payment_Gateway {
	...
	public function cloneRecorrenceOrder ($orderId, $orderStatus = 'processing') {
	    ...
	    //Obtendo dados da compra a ser copiada
        $OldOrder = wc_get_order( $orderId );
        $metaOld = get_post_meta( $orderId );

        //Criando base para nova compra que esta sendo criada baseada em outra ja existente
        $newOrder = wc_create_order();

        $newOrderId = $newOrder->get_id();

        //Configurando todos os campos customizados ou não da compra
        foreach ($metaOld as $currentFieldLKey => $currentValue) {

            update_post_meta($newOrderId, $currentFieldLKey, $currentValue['0']);

        }

        //Adicionando todos os itens pertencentes a compra
        foreach ( $OldOrder->get_items() as $item ) {
        ...
        $ship_rate_ob->id= 0;
        $ship_rate_ob->label= $OldOrder->get_shipping_method();
        $ship_rate_ob->taxes= $OldOrder->get_shipping_tax(); //not required in your situation
        $ship_rate_ob->cost= $OldOrder->get_shipping_total(); //not required in your situation
        $newOrder->add_shipping($ship_rate_ob);

        $newOrder->calculate_totals();

        $newOrder->update_status(
            $orderStatus, 
            'Compra gerada por pagamento recorrente baseado no pedido de nº ' . (int) $OldOrder->get_id() . 
            ' | Código da assinatura: ' . $OldOrder->get_meta('assinatura_hash'), 
            true
        );
        return $newOrderId;
	...
	public function process_payment( $order_id ) {
		...
		if ( 'lightbox' !== $this->method ) {
			if ( isset( $_POST['pagseguro_sender_hash'] ) && 'transparent' === $this->method ) { // WPCS: input var ok, CSRF ok.
				$response = $this->api->do_payment_request( $order, $_POST, $order_id ); // WPCS: input var ok, CSRF ok.

                /*if ($_POST['pagseguro_payment_method'] == 'recorrence') {

                    $update = wc_get_order( $order_id );
     ...
     * @throws WC_Data_Exception
     */
	public function recorrenceNotification ($orderId) {

        /*
         * Verificação justificada pois na primeira aprovação, será necessário que o pedido ja existente seja
         * atualizado. Este que foi criado juntamente com o plano e a assinatura.
         * A partir da segunda aprovação do plano em diante, será necessária a criação de um novo pedido como forma
         * de notificação e manutenção do flxuo do woocomerce.
         *
         * Para essa verificação, obtenho o id da compra original que é informado no campo 'recorrence' do plano
         * e verifico se seu status está pedente 'pending'. Se verdadeiro, significa que é a primeira aprovação do
         * plano.
         *
         */
        ...
        if (wc_get_order($orderId)->get_status() != 'processing') {
            ...
            return $this->cloneRecorrenceOrder($orderId);
			...
			if ( $order_id === $id ) {
				...
				switch ( intval( $posted->status ) ) {
					...
					case 3:
						...
						if (get_post_meta($order_id, (string) $posted->code, true) != '') {

							exit;

						} else if (get_post_meta($order_id, 'recorrence', true) == '1') {

							update_post_meta($order_id, (string) $posted->code, (int) $this->recorrenceNotification($order_id));
